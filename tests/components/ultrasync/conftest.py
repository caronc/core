...
def ultrasync_api(hass):
    ...
    with patch("ultrasync.UltraSync") as mock_api:
        ...
        instance.details = AsyncMock(side_effect=MOCK_RESPONSES)
        instance.areas = AsyncMock(return_value=list(MOCK_AREAS_0))
        instance.zones = AsyncMock(return_value=list(MOCK_ZONES_0))
        yield mock_api
